...
function cloudTasks(): SystemTask[] {
  ...
  if (!hasSupabaseEnv()) {
    ...
    tasks.push({
      ...
      description:
        "Run supabase db push or apply migrations 0001–0016 in order. Verify RLS is enabled on all business tables.",
      category: "cloud",
      priority: "critical",
      status: "blocked",
      source: "cloud_check",
...
function masterDataTasks(): SystemTask[] {
  return [
    ...
    },
    {
      ...
      title: "Configure portal access & create first users",
      description:
        "Use the in-app user console to create portal accounts. Set SUPABASE_SERVICE_ROLE_KEY in .env.local, then create technician, doctor, and accountant logins directly — no Supabase dashboard UID copy-paste needed.",
      category: "setup",
      priority: "high",
      status: "open",
      source: "setup_wizard",
    ...
    {
      id: "identity-configure-admin-key",
      title: "Configure server admin key",
      description:
        "Add SUPABASE_SERVICE_ROLE_KEY to .env.local (server-only, never NEXT_PUBLIC). This enables direct account creation without the Supabase dashboard. The key was provided during project setup.",
      category: "setup",
      priority: "high",
      status: "open",
      source: "setup_wizard",
    ...
    {
      id: "identity-create-technician-login",
      title: "Create first technician login",
      description:
        "Go to Command Center → Users → Create user. Set role to Technician, link their technician record, set a temporary password. Share credentials securely.",
      category: "pilot",
      priority: "high",
      status: "open",
      source: "setup_wizard",
      actionHref: "/command-center/users/new",
      recommendedOwner: "owner",
      recommendedModel: "haiku",
      blocksPilot: true,
      blocksProduction: false,
    ...
    {
      id: "identity-create-doctor-login",
      title: "Create first doctor login",
      description:
        "Go to Command Center → Users → Create user. Set role to Doctor, link their doctor record, set a temporary password. Verify they can see only their own cases.",
      category: "pilot",
      priority: "high",
      status: "open",
      source: "setup_wizard",
    ...
    {
      id: "identity-create-accountant-login",
      title: "Create accountant login",
      description:
        "Create a portal account with the Accountant role. The accountant can view invoices, payments, and doctor statements but cannot move production stages or see doctor portal data.",
      category: "pilot",
      priority: "medium",
      status: "open",
      source: "setup_wizard",
    ...
    {
      id: "identity-review-permissions",
      title: "Review permissions console",
      description:
        "Open Roles & Access → Portal destinations. Confirm each role has the right access for your lab before sharing credentials with team members.",
      category: "pilot",
      priority: "medium",
      status: "open",
      source: "setup_wizard",
    ...
    {
      id: "identity-add-email-provider",
      title: "Add email invite provider (Phase 14)",
      description:
        "Configure Resend API key to enable email invitations and password reset links. Until then, share credentials manually and securely.",
      category: "pilot",
      priority: "low",
      status: "open",
      source: "setup_wizard",
    ...
    {
      id: "identity-audit-log-review",
      title: "Review access audit logs",
      description:
        "Check Command Center → Users for the recent activity panel. Verify all account creation and deactivation events are recorded correctly before going live.",
      category: "pilot",
      priority: "low",
      status: "open",
      source: "setup_wizard",
...
function pilotTasks(): SystemTask[] {
  return [
    ...
    },
    {
      ...
      description:
        "Go to Command Center → Users → Create user. Set role to Technician, link their technician record, and set a temporary password. No Supabase Dashboard access required.",
      category: "pilot",
      priority: "high",
      status: "open",
      source: "setup_wizard",
    ...
    {
      id: "pilot-doctor-portal-case-submit",
      title: "Doctor submits first case from portal",
      description:
        "Log in as a linked doctor, open New case, select clinic from their linked clinics, choose an operation and material from the lab catalog, pick teeth on the FDI chart, and submit. Confirm the case appears in the lab case list with missing-info check applied.",
      category: "pilot",
      priority: "high",
      status: "open",
      source: "setup_wizard",
      actionHref: "/doctor-portal/cases/new",
      recommendedOwner: "owner",
      recommendedModel: "haiku",
      blocksPilot: true,
      blocksProduction: false,
    ...
    {
      id: "identity-test-email-invite",
      title: "Test email invite flow end-to-end",
      description:
        "Requires RESEND_API_KEY. Create a user with 'Save invite record' mode + 'Send invite email' checked. Open the invite link from email, verify the /auth/accept-invite page shows correct role and expiry, then sign in.",
      category: "pilot",
      priority: "low",
      status: "open",
      source: "setup_wizard",
    ...
    {
      id: "identity-test-password-reset",
      title: "Test password reset flow",
      description:
        "Go to /auth/reset-password, enter a portal user email. Verify the reset email arrives (requires RESEND_API_KEY or Supabase default email). Click the link, set a new password, and confirm login succeeds.",
      category: "pilot",
      priority: "low",
      status: "open",
      source: "setup_wizard",
